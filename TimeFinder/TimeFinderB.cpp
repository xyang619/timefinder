...
int bSearchT(double (*fp1)(double, double, int, double), double (*fp2)(double, double, int, double), vector<double> segs1, vector<double> segs2, double m1, double m2, double cutoff, int maxT, double &llk);
...
int main(int argc, char ** argv)
{
    ...
    index[0] = bSearchT(fhi, fhi, segs[pop1], segs[pop2], m, 1 - m, cutoff, maxT, globalMax[0]);
    index[1] = bSearchT(fga, fga, segs[pop1], segs[pop2], m, 1 - m, cutoff, maxT, globalMax[1]);
    index[2] = bSearchT(fcgfr, fcgfd, segs[pop1], segs[pop2], m, m, cutoff, maxT, globalMax[2]);
    index[3] = bSearchT(fcgfd, fcgfr, segs[pop1], segs[pop2], 1 - m, 1 - m, cutoff, maxT, globalMax[3]);
...
int bSearchT(double (*fp1)(double, double, int, double), double (*fp2)(double, double, int, double), vector<double> segs1, vector<double> segs2, double m1, double m2, double cutoff, int maxT, double &llk)
{
    ...
    if (maxT == 2)
    {
        double llk1 = loglik(fp1, segs1, m1, 1, cutoff) + loglik(fp2, segs2, m2, 1, cutoff);
        double llk2 = loglik(fp1, segs1, m1, 2, cutoff) + loglik(fp2, segs2, m2, 2, cutoff);
        if (llk1 > llk2)
        {
            llk = llk1;
            return 1;
    ...
    while (1)
    {
        ...
        lmiddle = loglik(fp1, segs1, m1, tmiddle, cutoff) + loglik(fp2, segs2, m2, tmiddle, cutoff);
        lleft = loglik(fp1, segs1, m1, tmiddle - 1, cutoff) + loglik(fp2, segs2, m2, tmiddle - 1, cutoff);
        lright = loglik(fp1, segs1, m1, tmiddle + 1, cutoff) + loglik(fp2, segs2, m2, tmiddle + 1, cutoff);
