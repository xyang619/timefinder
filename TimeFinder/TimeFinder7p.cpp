...
#pragma omp parallel for
    for (t = 1; t <= maxT; ++t)
    {
        llk[0][t - 1] = loglik(fhi, segs[pop1], m, t, cutoff, length) + loglik(fhi, segs[pop2], 1 - m, t, cutoff, length);
        llk[1][t - 1] = loglik(fga, segs[pop1], m, t, cutoff, length) + loglik(fga, segs[pop2], 1 - m, t, cutoff, length);
        llk[2][t - 1] = loglik(fcgfr, segs[pop1], m, t, cutoff, length) + loglik(fcgfd, segs[pop2], m, t, cutoff, length); //1 as recepient
        llk[3][t - 1] = loglik(fcgfd, segs[pop1], 1 - m, t, cutoff, length) + loglik(fcgfr, segs[pop2], 1 - m, t, cutoff,length); //1 as donor
    }
