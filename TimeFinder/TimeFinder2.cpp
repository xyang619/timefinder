...
int main(int argc, char ** argv) {
	...
	double *byPop1 = new double[6];
	for (int i = 0; i < 6; ++i) {
		byPop1[i] = globalMax[2 * i];
	}
	int mod = indexOfMax(byPop1, 6);
	delete[] byPop1;
	if (mod / 2 == 1) {
		int t = index[2];
		...
		if (llk[1][t1] >= llk[5][t2]) {
			cout << "model is HI, generation is " << (t1 + 1) << ", likelihood is " << llk[0][t1] << endl;
		} else {
			cout << "model is CGF, generation is " << (t2 + 1) << ", likelihood is " << llk[4][t2] << endl;
		}
	}
